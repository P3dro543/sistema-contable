...
// ===== CARGAR TERCEROS DESDE LA BD =====
function cargarTerceros() {
    fetch(BASE_URL + 'AsientosController/getTerceros')
        .then(response => response.json())
        .then(data => {
            terceros = data;
        });
...
function generarDetalleHTML(detalle, idAsiento) {
    ...
    detalle.forEach(linea => {
        ...
        html += `<td>
            <button class="btn btn-sm btn-warning asignar-tercero" 
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('asignar-tercero')) {
        ...
        .then(distribucionExistente => {
            ...
            if (distribucionExistente && distribucionExistente.length > 0) {
                distribucionExistente.forEach(item => {
                    agregarFilaDistribucion(item.id_tercero, item.monto);
                });
            } else {
...
function agregarFilaDistribucion(idTerceroSeleccionado = '', monto = '') {
    ...
    let options = '<option value="">Seleccione tercero</option>';
    terceros.forEach(t => {
        const selected = (idTerceroSeleccionado == t.id_tercero) ? 'selected' : '';
        options += `<option value="${t.id_tercero}" ${selected}>${t.identificacion} - ${t.nombre}</option>`;
    });
    
    fila.innerHTML = `
        <div class="col-md-6">
            <select class="form-select tercero-select">${options}</select>
        </div>
        <div class="col-md-4">
...
function guardarProrrateo() {
    ...
    filas.forEach(fila => {
        const select = fila.querySelector('.tercero-select');
        const monto = fila.querySelector('.monto-input');
        
        if (select.value && monto.value) {
            distribucion.push({
                id_tercero: parseInt(select.value),
