...
class Tercero
{
    ...
    // Listar terceros activos (para selects / asignaciones)
    public function getTercerosActivos($search = '')
    {
        $sql = "SELECT id_tercero, identificacion, nombre FROM terceros WHERE estado = 1 ";
        if (!empty($search)) {
            $sql .= "AND (nombre LIKE :search OR identificacion LIKE :search) ";
        }
